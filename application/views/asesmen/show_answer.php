										...
										<div>
												...
												<div class="mt-3" style="line-height: 2;">
													...
													<?php foreach ($detail['exam_answer'] as $key => $valEssay) : ?>
														...
														<?php if (
															...
															<p>
																<i class="fa-solid fa-check-circle text-success me-2"></i>
																<span class="d-inline"> <?= strtolower($valEssay['exam_answer']) ?> </span>
															</p>
														<?php else : ?>
											...
											<?php if ($detail['type'] == 6) : ?>
												...
												<?php foreach($detail['exam_answer'] as $key => $val): ?>
													...
													<?php foreach(json_decode($val['exam_answer'], true) as $key2 => $val2): ?>

														<!-- Looping jawaban benar -->
														<?php foreach(json_decode($val['correct_answer'], true) as $key3 => $val3): ?>
															<?php if($val2['urutan'] == $val3['urutan']): ?>
																<p style="margin-bottom: 5px;"><i class="fa fa-dot-circle-o me-2"></i><?=$val3['answer_correct']?></p>
															<?php endif ?>
														<?php endforeach ?>
