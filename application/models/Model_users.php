<?php
  class Model_users extends CI_Model {
	...
	public function getUsers(array $filter = NULL, int $limit = NULL, int $offset = NULL) {
		$query = "SELECT a.userid, a.username, a.password, a.user_level, b.user_level_name, a.active, a.sekolah_id, s.sekolah_nama, s.sekolah_kode
				  FROM users a 
				  LEFT JOIN user_level b ON a.user_level=b.user_level_id
				  LEFT JOIN sekolah s ON s.sekolah_id=a.sekolah_id";
		if(!empty($filter[2]['search']['value']) ||
	...
	/**
	 * Get Shpooing Cart Items By User
	 *
	 * @return void
	 */
	public function getShopingItemsByUser($user) {
		$query = "SELECT a.book_code, a.title, a.cover_img, a.author, a.isbn, a.publisher_id, a.file_1, b.publisher_name
				  FROM ebooks a, publishers b, shopping_cart c
				  WHERE a.publisher_id=b.id AND a.id=c.item_id AND c.user_id=?";
