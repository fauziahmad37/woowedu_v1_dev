...
const PDFViewerApplication = {
  ...
  async _initializeViewerComponents() {
    ...
    if (appConfig.sidebar && appConfig.sidebar?.sidebarContainer) {
  ...
  downloadOrSave(options = {}) {
    ...
    // Tambahan
  async saveDefaultFileToServer(options = {}) {
      // eslint-disable-next-line no-unused-vars
      const url = this._downloadUrl,
      // eslint-disable-next-line no-unused-vars
      filename = this._docFilename,
      statusDialog = this.appConfig.successStatusDialog;

      statusDialog.loadingDialog.show();
    try {
      this._ensureDownloadComplete();
      ...
      formData.append("pdfFile", new File([blob], options.filename));
      formData.append('bookId', options.book);
 
      const f = await fetch(options.serverUrl, {
        method: "POST",
        body: formData,
        mode: "no-cors",
      });
      ...
      statusDialog.loadingDialog.close();

      if(j.message) {
        statusDialog.dialog.show();
        statusDialog.dialog.dataset.error = j.success == true ? "success" : "error";
        statusDialog.message.innerHTML = j.message
      }
      ...
      if(err.message) {
        statusDialog.dialog.show();
        statusDialog.dialog.dataset.error = "error";
        statusDialog.message.innerHTML = err.message
      }
  ...
  // Tambahan
  async saveWithAnnotationToServer(options = {}) {
    const annotations = this.pdfDocument?.annotationStorage,
          statusDialog = this.appConfig.successStatusDialog;

    const anno = annotations.serializable.map;
    ...
    try {
      ...
      const formData = new FormData();
      formData.append("pdfFile", new File([blob], options.filename));
      formData.append("annotation-info", JSON.stringify(Object.fromEntries(anno)));
      formData.append('bookId', options.book);
      ...
      if(reason.message) {
        statusDialog.dialog.show();
        statusDialog.dialog.dataset.error = "error";
        statusDialog.message.innerHTML = reason.message
      }

      console.error(`Error when saving the document: ${reason.message}`);
      await this.saveDefaultFileToServer(options);
    } finally {
      await this.pdfScriptingManager.dispatchDidSave();
      this._saveInProgress = false;
      setTimeout(() => {
  ...
  async saveToServer(options) {
    if (this.pdfDocument?.annotationStorage.size > 0) {
      this.saveWithAnnotationToServer(options);
    } else {
      this.saveDefaultFileToServer(options);
    }
  },
...
class AppOptions {
  ...
  static setDefault(name, valueDefault) {
    defaultOptions[name].value = valueDefault;
  }
...
function getViewerConfiguration() {
  return {
    ...
    successStatusDialog: { 
      dialog: document.getElementById('successStatusDialog'),
...
function webViewerLoad() {
  ...
  PDFViewerApplicationOptions.set("locale", "id");
  PDFViewerApplication.open({ url: appInfo.file })
