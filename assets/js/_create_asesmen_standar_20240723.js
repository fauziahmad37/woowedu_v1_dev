...
$(document).ready(function () {
	...
	$('button.pilih-pertanyaan-1').on('click', function(e){
		...
		var card = document.querySelector('.content-1').getElementsByClassName('soal-id');
	...
	$('button.pilih-pertanyaan-2').on('click', function(e){
		...
		var card = document.querySelector('.content-2').getElementsByClassName('soal-id');
		mapp = [...card].map(val => parseInt(val.value));
		console.log(mapp);
	...
	$('#tabelPilihSoal > tbody').on('click', '.btn.add_soal', e => {
		...
		if(jenisSoal == 1){
			nomor_pg++;
			htmlContent = pilihanGanda(buttonPilihAktif, nomor_pg, item);
		} else {
			nomor_essay++;
			htmlContent = essay(buttonPilihAktif, nomor_essay, item);
		}
...
async function postAssesment(e, is_draft=0, redirect='') {
	...
	try 
	{
		...
		// .then(t => {
		// 		if(resp.id)
		// 			window.location.href = redirect + '/' + resp.id;
		// });		
	} 
	catch (error) {
		Swal.fire({
