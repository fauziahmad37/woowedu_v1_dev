 ...
 * @type {*}
 */
let url = (classId) ? 'materi/list' : 'materi/list_materi_sekolah' 
const table = $('#tbl-materi').DataTable({
    ...
    columns: [
		...
		{
            data: 'teacher_name',
			visible: (classId) ? true : false
        },
		{
			data: null,
			render(data, type, row, meta){
				return (row.edit_at) ? moment(row.edit_at).format('D MMM YYYY, H:m') : '';
			}
		},
		{
            data: null,
...
const getSubject = async () => {
    try
    {
        const url = new URL(`${ADMIN_URL}api/subject/getAll?sekolah_id=${sekolah_id}&class_level_id=${classLevelId}`);
...
(async ($, table) => {
	...
	// jika user level murid atau orang tua. kalau guru atau ortu tidak perlu
	if(userLevel == 4 || userLevel == 5){
		const materi = [...(await getSubject()).data].map(x => ({ id: x.id_mapel, text: x.nama_mapel }));

		// Filter hanya ada di login murid dan orang tua
