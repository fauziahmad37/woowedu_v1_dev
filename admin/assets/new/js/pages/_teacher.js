(($, base_url) => {
	 ...
	 */
		document.getElementById('btn-search_teacher').addEventListener('click', e => {
			...
			if(typeof class_name.value !== 'undefined' || class_name.value !== null)
					table.column(5).search(class_name.value).draw();
	});
	...
	*/
	...
	/**
	 * IMPORT DATA SUBJECT TEACHERS
	 */

	let impFormSubject = document.forms['import-subject-teacher'];
	let impSubjectFormat = impFormSubject['import-format'],
		impSubjectFile = impFormSubject['file-upload'];

	let importSubjectFormat = null,
		importSubjectFile = null;
	impSubjectFile.addEventListener('change', e => {
		e.preventDefault();
		let lbl = document.querySelectorAll('.custom-file-label')[1];
		lbl.innerHTML = e.target.files[0].name;
		impSubjectFile = e.target.files[0];
	});
	impFormSubject.addEventListener('submit', e => {
		e.preventDefault();
		let fdata = new FormData();
		fdata.append('format', impSubjectFormat.value);
		fdata.append('upload-file', impSubjectFile);
		// upload 
		$.ajax({
			url: base_url + 'api/subject/importSubjectTeacher',
			type: 'POST',
			data: fdata,
			contentType: false,
			processData: false,
			...
			complete() {
				table.ajax.reload();
				$('#modal-import-subject-teacher').modal('hide');
			}
		});
